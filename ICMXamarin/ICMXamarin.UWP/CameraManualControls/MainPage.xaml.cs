...
namespace CameraManualControls
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        protected override async void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            fala = (ICMXamarin.UWP.Fala)e.Parameter;
        }
        ...
        private async void PreviewControl_Tapped(object sender, TappedRoutedEventArgs e)
        {
            ...
            var img = await TakePhotoAsync();
            ImgFotoTirada.Source = img;
        }
        ...
        /// </summary>
        /// <returns></returns>
        private async Task<WriteableBitmap> TakePhotoAsync()
        {
            fala.Falar("Identificando imagem.");
            ...
            WriteableBitmap imgSource = null;
            ...
            try
            {
                ...
                //imgSource = new WriteableBitmap(softwareBitmap.PixelWidth, softwareBitmap.PixelHeight);
                //var pixelData = await decoder.GetPixelDataAsync();
                ////var bytes = pixelData.DetachPixelData();
                //ImageStream xs;
                ////var preview = await decoder.GetFrameAsync(0);
                ////var outputStream = preview.AsStream();
                //softwareBitmap.CopyToBuffer(imgSource.PixelBuffer);

                //ImgFotoTirada.Source = imgSource;
                ...
                //string r = await ICMXamarin.Model.MSCognitiveServices.GetDescricaoImagemComputerVisionApi(bytes);
                //fala.Falar(r);
                //Debug.WriteLine(r);

                var photoOrientation = ConvertOrientationToPhotoOrientation(GetCameraOrientation());

                string nomeArq = await ReencodeAndSavePhotoAsync(stream, photoOrientation);

                var file = await KnownFolders.PicturesLibrary.OpenStreamForReadAsync(nomeArq);
                byte[] imgBytes = new byte[file.Length];
                file.Read(imgBytes, 0, imgBytes.Length);
            ...
            catch (Exception ex)
            {
                ...
                fala.Falar($"Ops. Um erro aconteceu: {ex.ToString()}");
            }
        ...
        private static async Task<string> ReencodeAndSavePhotoAsync(IRandomAccessStream stream, PhotoOrientation photoOrientation)
        {
            ...
            using (var inputStream = stream)
            {
                ...
                var file = await KnownFolders.PicturesLibrary.CreateFileAsync("imagem-desc.jpeg", CreationCollisionOption.GenerateUniqueName);
                nomeArq = file.Name;
